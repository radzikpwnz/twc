...
int GenerateEventHandler(TWC_OBJECT *obj, CONTROL_EVENT_INFO *cei, TCHAR **pbuf) {
	UINT len, type_len;
	TCHAR *p;
	int i;
...
int CreateCodeWindow( TWC_OBJECT *window, TCHAR *code)
{
	...
	if ( OBJ_CLIENT_DATA(window)->code_window == NULL) {
		OBJ_CLIENT_DATA(window)->code_window = (CODE_WINDOW *)malloc(sizeof(CODE_WINDOW));
		memset(OBJ_CLIENT_DATA(window)->code_window, 0, sizeof(CODE_WINDOW));
		OBJ_CLIENT_DATA(window)->code_window->window = window;
	}

	if (OBJ_CLIENT_DATA(window)->code_window->hwnd == NULL) {
		title = (TCHAR *)malloc((_tcslen(window->name) + 10) * sizeof(TCHAR));
		_mytcscpy(_mytcscpy(title, window->name), T(" - code"));
		CreateWindowEx( WS_EX_MDICHILD, T("CodeWnd"), title, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, 0, CW_USEDEFAULT, 0,
                        MDIClient.hwnd, NULL, GetModuleHandle(NULL), OBJ_CLIENT_DATA(window)->code_window);
		free(title);
		if (OBJ_CLIENT_DATA(window)->code_window->hwnd == NULL) {
			return 0;
		}
