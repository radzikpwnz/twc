...
static int PaintControlSelectionFrame( TWC_OBJECT *obj)
{
    PAINTSTRUCT ps;
    HDC hDC;
    HBRUSH hOldBrush;
    ...
    hDC = BeginPaint( OBJ_CLIENT_DATA( obj)->static_hwnd, &ps);

    if ( OBJ_CLIENT_DATA( obj)->selected ) {
        hDC = GetWindowDC( OBJ_CLIENT_DATA( obj)->static_hwnd);
        GetWindowRect( OBJ_CLIENT_DATA( obj)->static_hwnd, &rect);
        MapWindowPoints( HWND_DESKTOP, OBJ_CLIENT_DATA( obj)->static_hwnd, (POINT *)&rect, 2);
        ...
        DeleteObject( hPen);
        ReleaseDC( OBJ_CLIENT_DATA( obj)->static_hwnd, hDC);
    }

    EndPaint( OBJ_CLIENT_DATA( obj)->static_hwnd, &ps);
...
LRESULT CALLBACK StaticWndProc( HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch ( msg ) {
        case WM_NCDESTROY:
            ...
            OBJ_CLIENT_DATA( obj)->static_hwnd = NULL;
            break;
        case WM_SETCURSOR:
            return 1;
        case WM_PAINT:
    ...
    return CallWindowProc( OBJ_CLIENT_DATA( obj)->static_orig_wndproc, hwnd, msg, wParam, lParam);
}